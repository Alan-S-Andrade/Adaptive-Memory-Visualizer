...
/*int main(int argv, char ** argc) {
	...
	datastructure * ds = malloc(sizeof(datastructure) * 3);
	
	ds[0].name = malloc(20);
	strcpy(ds[0].name, "Struct 1");
	ds[0].std_structure = malloc(20);
	strcpy(ds[0].std_structure, "queue");
	ds[1].name = malloc(20);
	strcpy(ds[1].name, "Struct 2");
	ds[2].name = malloc(20);
	strcpy(ds[2].name, "Struct 3");
	
	writeHead(out);
	writeSVG(out, ds, NULL, 3);
	writeSummary(out);
	fclose(out);
